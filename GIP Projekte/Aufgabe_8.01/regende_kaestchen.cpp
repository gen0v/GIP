...
void handle_mouse_click(Box boxes[])
{
	...
	for (int i = 0; i < box_max; i++)
	{
		if (mouse_x >= boxes[i].x &&
			mouse_x <= boxes[i].x + box_size &&
			mouse_y >= boxes[i].y &&
			mouse_y <= boxes[i].y + box_size) {
			boxes[i].delta_y += 10;
			boxes[i].y = 0-boxes[i].delta_y;//minus delta da sonst im naechsten schritt schon delta aufaddiert wurde
		}
	}
}
int main()
{
	...
	while (keep_going && gip_window_not_closed())
	{
		...
		{
			gip_wait(1); // warte 5 Milli-Sekunden
